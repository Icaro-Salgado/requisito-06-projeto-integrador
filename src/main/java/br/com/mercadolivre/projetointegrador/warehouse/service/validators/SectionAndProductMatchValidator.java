...
public class SectionAndProductMatchValidator implements ModelValidator {
    ...
    public static void validate(Product product, Section section) throws SectionDoesNotMatchWithProductException{
        if (!section.getProduct_category().equals(product.getCategory())) {
            throw new SectionDoesNotMatchWithProductException(
                    "A seção aceita a categoria  "
                            + section.getProduct_category()
                            + " mas foram encontrados as seguintes categorias "
                            + product.getCategory());
        }
    }
